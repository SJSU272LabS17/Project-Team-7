...
function changeUser(company, parent, pos)
{
    ...
    $('#userDets').html('<span id="username" >'+config.participants.users[parent][pos].user+'</span> (<span id="userType">'+config.participants.users[parent][pos].label+'</span>: <span id="company">'+config.participants.users[parent][pos].company+'</span>)');
    toggleMenu();
    let now = new Date();
    let time = now.getTime() + 4*3600 * 1000;
    now.setTime(time);
	...
	*/
    ...
    sendDets = config.participants.users[parent][pos].label + ": " +config.participants.users[parent][pos].company;
}
...
function errCreate(v5cID)
{
    ...
    $('#failTransfer').show();
    $('#failTxt').html('Failed to create claim with Claim ID: '+v5cID);
    $('#loader').hide();
    $('#loaderMessages').html('');
}
...
function errTrans(numErrs, numSucc)
{
    ...
    $('#failTransfer').show();
    $('#failTxt').html('Completed with some errors: <br /><br />'+numErrs+' claims failed.<br />'+numSucc+' claims were successful');
    $('#loader').hide();
    $('#loaderMessages').html('');
}
...
{
    $('#failHd span').html('Settlement Failed');
    $('#failTransfer').show();
    $('#failTxt').html('Completed with some errors: <br /><br />'+numErrs+' claims failed.<br />'+numSucc+' claims were successful');
    $('#loader').hide();
    $('#loaderMessages').html('');
