			...
			if(array[i].trim() != "")
			{
				...
				if(!found)
				{
					if(pgNm == "Regulator")
					{
						...
						{
							obj.VIN = '&lt;<i>VIN</i>&gt;';
							obj.make = '&lt;<i>Licence Plate Number</i>&gt;';
							obj.model = '&lt;<i>Accident Date</i>&gt;';
							obj.colour = '&lt;<i>Policy ID</i>&gt;';
							obj.reg = '&lt;<i>Amount</i>&gt;';
							objects.push(obj);
						}
					}
					...
					{
						error = true
						$("#vhclsTbl").append("Unable to load claims.");
					}
				}
			}
		}
	...
	xhr.onprogress = function () {
		...
		for(var i = 0; i < array.length; i++)
		{
			if(array[i].trim() != "")
			{
				...
				console.log("UPDATE Claim READ:", obj)
	...
	xhr.onreadystatechange = function (){
		if(xhr.readyState === 4)
		{
			...
			for(var i = 0; i < d.length; i++)
			{
				...
				if(data.make.toLowerCase() == 'undefined' || data.make.trim() == '') data.make = '&lt;<i>Licence Plate Number</i>&gt;';
				if(data.model.toLowerCase() == 'undefined' || data.model.trim() == '') data.model = '&lt;<i>Accident date</i>&gt;';
				if(data.reg.toLowerCase() == 'undefined' || data.reg.trim() == '') data.reg = '&lt;<i>Amount</i>&gt;';
				if(data.colour.toLowerCase() == 'undefined' || data.colour.trim() == '') data.colour = '&lt;<i>Policy ID</i>&gt;';
