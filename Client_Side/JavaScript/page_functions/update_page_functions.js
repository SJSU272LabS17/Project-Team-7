...
function showEditTbl(el)
{
	...
	if(make == '&lt;<i>Licence Plate Number</i>&gt;')
	{
		make = 'undefined'
	}
	var model = $(el).siblings('.carModel').html()
	if(model == '&lt;<i>Accident date</i>&gt;')
	{
		model = 'undefined'
	}
	var colour = $(el).siblings('.carColour').html()
	if(colour == '&lt;<i>Policy ID</i>&gt;')
	{
		colour = 'undefined'
	}
...
function validate(el)
{
	...
	*/
	...
	if($('#make').val().trim() == '')
	{
		$('#errorRw').find('ul').append('<li>Licence plate number cannot be blank</li>')
		failed = true;
	}
	if($('#make').val().trim().toLowerCase() == 'undefined' && $('#hidMake').val().trim().toLowerCase() != 'undefined')
	{
		$('#errorRw').find('ul').append('<li>Licence plate number cannot be reset to undefined</li>')
		failed = true;
	}
	if($('#model').val().trim() == '')
	{
		$('#errorRw').find('ul').append('<li>Accident Date cannot be blank</li>')
		failed = true;
	}
	if($('#model').val().trim().toLowerCase() == 'undefined' && $('#hidModel').val().trim().toLowerCase() != 'undefined')
	{
		$('#errorRw').find('ul').append('<li>Accident Date cannot be reset to undefined</li>')
		failed = true;
	}
	if($('#colour').val().trim() == '')
	{
		$('#errorRw').find('ul').append('<li>Policy ID cannot be blank</li>')
		failed = true;
	}
	if($('#colour').val().trim().toLowerCase() == 'undefined' && $('#hidColour').val().trim().toLowerCase() != 'undefined')
	{
		$('#errorRw').find('ul').append('<li>Policy ID cannot be reset to undefined</li>')
		failed = true;
	}
	if($('#reg').val().trim() == '')
	{
		$('#errorRw').find('ul').append('<li>Amount cannot be blank</li>')
		failed = true;
	}
	if($('#reg').val().trim().toLowerCase() == 'undefined' && $('#hidReg').val().trim().toLowerCase() != 'undefined')
	{
		$('#errorRw').find('ul').append('<li>Amount cannot be reset to undefined</li>')
		failed = true;
	}
	if(!failed)
	{
